...
class Fitxategia extends ModelAbstract
{
    /*
     * @var \Iron_Model_Fso
     */
    ...
    protected $_downloadMotaAcceptedValues = array(
        'audio',
        'video',
        'text',
        'calc',
    ...
    /**
     * [enum:audio|video|text|calc]
     * Database var type varchar
     *
     * @var string
     */
    protected $_downloadMota;


    /**
     * Parent relation fk_Fitxategia_Edukia1
     *
    ...
    public function __construct()
    {
        $this->setColumnsMeta(array(
            ...
            'mota'=> array('enum:mega|bestelakoak'),
            'downloadMota'=> array('enum:audio|video|text|calc'),
        ));
    ...
    ***************************************************************************/
     ...
     * @param int $data
     * @return \Klikasi\Model\Raw\Fitxategia
     */
     ...
     * @return \Klikasi\Model\Raw\Fitxategia
     */
    public function setDownloadMota($data)
    {

        if ($this->_downloadMota != $data) {
            $this->_logChange('downloadMota');
        }

        if ($data instanceof \Zend_Db_Expr) {
            $this->_downloadMota = $data;

        } else if (!is_null($data)) {
            if (!in_array($data, $this->_downloadMotaAcceptedValues) && !empty($data)) {
                throw new \InvalidArgumentException(_('Invalid value for downloadMota'));
            }
            $this->_downloadMota = (string) $data;
    ...
    /**
     * Gets column downloadMota
     *
     * @return string
     */
    public function getDownloadMota()
    {
        return $this->_downloadMota;
    }
     ...
     *
     * @return Klikasi\Mapper\Sql\Fitxategia
     */
    public function getMapper()
    {
        if ($this->_mapper === null) {
            ...
            if (class_exists('\Klikasi\Mapper\Sql\Fitxategia')) {

                $this->setMapper(new \Klikasi\Mapper\Sql\Fitxategia);
     ...
     *
     * @return null | \Klikasi\Model\Validator\Fitxategia
     */
    public function getValidator()
    {
        if ($this->_validator === null) {

            if (class_exists('\Klikasi\\Validator\Fitxategia')) {

                $this->setValidator(new \Klikasi\Validator\Fitxategia);
            }
        }
     ...
     *
     * @see \Mapper\Sql\Fitxategia::delete
