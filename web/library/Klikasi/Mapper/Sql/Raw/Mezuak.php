...
class Mezuak extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Mezuak';
     ...
     *
     * @param Klikasi\Model\Raw\Mezuak $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Mezuak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Mezuak object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Mezuak object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                'id' => $model->getId(),
                'norkId' => $model->getNorkId(),
                'noriId' => $model->getNoriId(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Mezuak
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Mezuak');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Mezuak $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Mezuak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Mezuak object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Mezuak object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Mezuak', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Mezuak $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Mezuak $model, $useTransaction = true,
     ...
     * @param Klikasi\Model\Raw\Mezuak|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Mezuak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Mezuak();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setNorkId($data['norkId'])
                  ->setNoriId($data['noriId'])
                  ->setMezua($data['mezua'])
                  ->setMota($data['mota'])
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Mezuak) {
            $entry->setId($data->getId())
                  ->setNorkId($data->getNorkId())
                  ->setNoriId($data->getNoriId())
                  ->setMezua($data->getMezua())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Mezuak');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Mezuak');
        }
