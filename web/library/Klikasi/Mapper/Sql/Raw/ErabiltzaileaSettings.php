...
/**
 * Data Mapper implementation for Klikasi\Model\ErabiltzaileaSettings
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class ErabiltzaileaSettings extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\ErabiltzaileaSettings';
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileaSettings $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileaSettings) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\ErabiltzaileaSettings object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\ErabiltzaileaSettings object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'moderazioBerria' => $model->getModerazioBerria(),
                'ikastegiakBerria' => $model->getIkastegiakBerria(),
                'ikasleEskaera' => $model->getIkasleEskaera(),
                'kolaborazioEskaera' => $model->getKolaborazioEskaera(),
                'edukiBerria' => $model->getEdukiBerria(),
                'iruzkinBerria' => $model->getIruzkinBerria(),
                'iradokizunBerria' => $model->getIradokizunBerria(),
                'gustokoBerria' => $model->getGustokoBerria(),
                'atseginDut' => $model->getAtseginDut(),
                'edukiariSalaketa' => $model->getEdukiariSalaketa(),
                'mezuBerria' => $model->getMezuBerria(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaSettings
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaSettings');
        }
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileaSettings $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileaSettings) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\ErabiltzaileaSettings object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\ErabiltzaileaSettings object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaSettings', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\ErabiltzaileaSettings $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\ErabiltzaileaSettings $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\ErabiltzaileaSettings $model,
     ...
     * @param Klikasi\Model\Raw\ErabiltzaileaSettings|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\ErabiltzaileaSettings The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\ErabiltzaileaSettings();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setModerazioBerria($data['moderazioBerria'])
                  ->setIkastegiakBerria($data['ikastegiakBerria'])
                  ->setIkasleEskaera($data['ikasleEskaera'])
                  ->setKolaborazioEskaera($data['kolaborazioEskaera'])
                  ->setEdukiBerria($data['edukiBerria'])
                  ->setIruzkinBerria($data['iruzkinBerria'])
                  ->setIradokizunBerria($data['iradokizunBerria'])
                  ->setGustokoBerria($data['gustokoBerria'])
                  ->setAtseginDut($data['atseginDut'])
                  ->setEdukiariSalaketa($data['edukiariSalaketa'])
                  ->setMezuBerria($data['mezuBerria']);
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\ErabiltzaileaSettings) {
            $entry->setId($data->getId())
                  ->setErabiltzaileaId($data->getErabiltzaileaId())
                  ->setModerazioBerria($data->getModerazioBerria())
                  ->setIkastegiakBerria($data->getIkastegiakBerria())
                  ->setIkasleEskaera($data->getIkasleEskaera())
                  ->setKolaborazioEskaera($data->getKolaborazioEskaera())
                  ->setEdukiBerria($data->getEdukiBerria())
                  ->setIruzkinBerria($data->getIruzkinBerria())
                  ->setIradokizunBerria($data->getIradokizunBerria())
                  ->setGustokoBerria($data->getGustokoBerria())
                  ->setAtseginDut($data->getAtseginDut())
                  ->setEdukiariSalaketa($data->getEdukiariSalaketa())
                  ->setMezuBerria($data->getMezuBerria());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ErabiltzaileaSettings');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('ErabiltzaileaSettings');
        }
