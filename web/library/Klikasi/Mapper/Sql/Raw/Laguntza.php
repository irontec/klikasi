...
/**
 * Data Mapper implementation for Klikasi\Model\Laguntza
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Laguntza extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Laguntza';
     ...
     *
     * @param Klikasi\Model\Raw\Laguntza $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Laguntza) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Laguntza object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Laguntza object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Laguntza
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Laguntza');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Laguntza $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Laguntza) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Laguntza object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Laguntza object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Laguntza', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Laguntza $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Laguntza $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Laguntza $model,
     ...
     * @param Klikasi\Model\Raw\Laguntza|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Laguntza The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Laguntza();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Laguntza) {
            $entry->setId($data->getId())
                  ->setTitulua($data->getTitulua())
                  ->setLaguntza($data->getLaguntza())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Laguntza');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Laguntza');
        }
