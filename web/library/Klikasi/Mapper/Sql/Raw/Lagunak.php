...
/**
 * Data Mapper implementation for Klikasi\Model\Lagunak
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Lagunak extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Lagunak';
     ...
     *
     * @param Klikasi\Model\Raw\Lagunak $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Lagunak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Lagunak object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Lagunak object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'erabiltzaileaId1' => $model->getErabiltzaileaId1(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Lagunak
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Lagunak');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Lagunak $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Lagunak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Lagunak object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Lagunak object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Lagunak', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Lagunak $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Lagunak $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Lagunak $model,
     ...
     * @param Klikasi\Model\Raw\Lagunak|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Lagunak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Lagunak();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setErabiltzaileaId1($data['erabiltzaileaId1'])
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Lagunak) {
            $entry->setId($data->getId())
                  ->setErabiltzaileaId($data->getErabiltzaileaId())
                  ->setErabiltzaileaId1($data->getErabiltzaileaId1())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Lagunak');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Lagunak');
        }
