...
/**
 * Data Mapper implementation for Klikasi\Model\EtiketaRelEdukia
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class EtiketaRelEdukia extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\EtiketaRelEdukia';
     ...
     *
     * @param Klikasi\Model\Raw\EtiketaRelEdukia $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\EtiketaRelEdukia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\EtiketaRelEdukia object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\EtiketaRelEdukia object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\EtiketaRelEdukia
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\EtiketaRelEdukia');
        }
     ...
     *
     * @param Klikasi\Model\Raw\EtiketaRelEdukia $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\EtiketaRelEdukia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\EtiketaRelEdukia object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\EtiketaRelEdukia object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\EtiketaRelEdukia', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\EtiketaRelEdukia $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\EtiketaRelEdukia $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\EtiketaRelEdukia $model,
     ...
     * @param Klikasi\Model\Raw\EtiketaRelEdukia|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\EtiketaRelEdukia The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\EtiketaRelEdukia();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\EtiketaRelEdukia) {
            $entry->setId($data->getId())
                  ->setEtiketaId($data->getEtiketaId())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'EtiketaRelEdukia');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('EtiketaRelEdukia');
        }
