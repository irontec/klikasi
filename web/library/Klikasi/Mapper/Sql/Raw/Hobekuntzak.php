...
class Hobekuntzak extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Hobekuntzak';
     ...
     *
     * @param Klikasi\Model\Raw\Hobekuntzak $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Hobekuntzak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Hobekuntzak object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Hobekuntzak object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Hobekuntzak
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Hobekuntzak');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Hobekuntzak $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Hobekuntzak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Hobekuntzak object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Hobekuntzak object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Hobekuntzak', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Hobekuntzak $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Hobekuntzak $model, $useTransaction = true,
     ...
     * @param Klikasi\Model\Raw\Hobekuntzak|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Hobekuntzak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Hobekuntzak();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setHobekuntzak($data['hobekuntzak'])
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Hobekuntzak) {
            $entry->setId($data->getId())
                  ->setErabiltzaileaId($data->getErabiltzaileaId())
                  ->setEdukiaId($data->getEdukiaId())
                  ->setHobekuntzak($data->getHobekuntzak())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Hobekuntzak');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Hobekuntzak');
        }
