...
/**
 * Data Mapper implementation for Klikasi\Model\Bideoa
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Bideoa extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Bideoa';
     ...
     *
     * @param Klikasi\Model\Raw\Bideoa $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Bideoa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Bideoa object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Bideoa object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Bideoa
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Bideoa');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Bideoa $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Bideoa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Bideoa object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Bideoa object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Bideoa', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Bideoa $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Bideoa $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Bideoa $model,
     ...
     * @param Klikasi\Model\Raw\Bideoa|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Bideoa The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Bideoa();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Bideoa) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Bideoa');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Bideoa');
        }
