...
/**
 * Data Mapper implementation for Klikasi\Model\KanpainaRelIkastegiMota
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class KanpainaRelIkastegiMota extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\KanpainaRelIkastegiMota';
     ...
     *
     * @param Klikasi\Model\Raw\KanpainaRelIkastegiMota $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\KanpainaRelIkastegiMota) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\KanpainaRelIkastegiMota object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\KanpainaRelIkastegiMota object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'ikastetxeMotaId' => $model->getIkastetxeMotaId(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelIkastegiMota
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelIkastegiMota');
        }
     ...
     *
     * @param Klikasi\Model\Raw\KanpainaRelIkastegiMota $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\KanpainaRelIkastegiMota) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\KanpainaRelIkastegiMota object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\KanpainaRelIkastegiMota object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelIkastegiMota', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\KanpainaRelIkastegiMota $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\KanpainaRelIkastegiMota $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\KanpainaRelIkastegiMota $model,
     ...
     * @param Klikasi\Model\Raw\KanpainaRelIkastegiMota|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\KanpainaRelIkastegiMota The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\KanpainaRelIkastegiMota();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setIkastetxeMotaId($data['ikastetxeMotaId']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setKanpainaId($data->{'kanpainaId'})
                  ->setIkastetxeMotaId($data->{'ikastetxeMotaId'});

        } else if ($data instanceof \Klikasi\Model\Raw\KanpainaRelIkastegiMota) {
            $entry->setId($data->getId())
                  ->setKanpainaId($data->getKanpainaId())
                  ->setIkastetxeMotaId($data->getIkastetxeMotaId());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KanpainaRelIkastegiMota');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('KanpainaRelIkastegiMota');
        }
