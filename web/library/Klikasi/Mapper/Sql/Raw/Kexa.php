...
class Kexa extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Kexa';
     ...
     *
     * @param Klikasi\Model\Raw\Kexa $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Kexa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Kexa object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Kexa object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'konponduta' => $model->getKonponduta(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Kexa
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Kexa');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Kexa $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Kexa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Kexa object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Kexa object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Kexa', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Kexa $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Kexa $model, $useTransaction = true,
     ...
     * @param Klikasi\Model\Raw\Kexa|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Kexa The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Kexa();
        }
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Kexa');
