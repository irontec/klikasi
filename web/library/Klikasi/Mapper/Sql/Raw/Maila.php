...
/**
 * Data Mapper implementation for Klikasi\Model\Maila
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Maila extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Maila';
     ...
     *
     * @param Klikasi\Model\Raw\Maila $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Maila) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Maila object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Maila object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Maila
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Maila');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Maila $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Maila) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Maila object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Maila object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Maila', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Maila $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Maila $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Maila $model,
     ...
     * @param Klikasi\Model\Raw\Maila|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Maila The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Maila();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Maila) {
            $entry->setId($data->getId())
                  ->setIzena($data->getIzena())
                  ->setUrl($data->getUrl())
                  ->setGrupo($data->getGrupo());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Maila');
