...
/**
 * Data Mapper implementation for Klikasi\Model\EdukiaRelMaila
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class EdukiaRelMaila extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\EdukiaRelMaila';
     ...
     *
     * @param Klikasi\Model\Raw\EdukiaRelMaila $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\EdukiaRelMaila) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\EdukiaRelMaila object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\EdukiaRelMaila object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelMaila
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelMaila');
        }
     ...
     *
     * @param Klikasi\Model\Raw\EdukiaRelMaila $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\EdukiaRelMaila) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\EdukiaRelMaila object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\EdukiaRelMaila object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelMaila', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\EdukiaRelMaila $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\EdukiaRelMaila $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\EdukiaRelMaila $model,
     ...
     * @param Klikasi\Model\Raw\EdukiaRelMaila|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\EdukiaRelMaila The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\EdukiaRelMaila();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setMailaId($data['mailaId']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setEdukiaId($data->{'edukiaId'})
                  ->setMailaId($data->{'mailaId'});

        } else if ($data instanceof \Klikasi\Model\Raw\EdukiaRelMaila) {
            $entry->setId($data->getId())
                  ->setEdukiaId($data->getEdukiaId())
                  ->setMailaId($data->getMailaId());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'EdukiaRelMaila');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('EdukiaRelMaila');
        }
