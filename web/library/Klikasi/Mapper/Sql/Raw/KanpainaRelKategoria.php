...
/**
 * Data Mapper implementation for Klikasi\Model\KanpainaRelKategoria
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class KanpainaRelKategoria extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\KanpainaRelKategoria';
     ...
     *
     * @param Klikasi\Model\Raw\KanpainaRelKategoria $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\KanpainaRelKategoria) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\KanpainaRelKategoria object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\KanpainaRelKategoria object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelKategoria
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelKategoria');
        }
     ...
     *
     * @param Klikasi\Model\Raw\KanpainaRelKategoria $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\KanpainaRelKategoria) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\KanpainaRelKategoria object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\KanpainaRelKategoria object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelKategoria', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\KanpainaRelKategoria $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\KanpainaRelKategoria $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\KanpainaRelKategoria $model,
     ...
     * @param Klikasi\Model\Raw\KanpainaRelKategoria|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\KanpainaRelKategoria The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\KanpainaRelKategoria();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\KanpainaRelKategoria) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KanpainaRelKategoria');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('KanpainaRelKategoria');
        }
