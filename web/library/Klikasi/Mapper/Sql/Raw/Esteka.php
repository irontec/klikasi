...
/**
 * Data Mapper implementation for Klikasi\Model\Esteka
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Esteka extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Esteka';
     ...
     *
     * @param Klikasi\Model\Raw\Esteka $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Esteka) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Esteka object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Esteka object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Esteka
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Esteka');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Esteka $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Esteka) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Esteka object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Esteka object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Esteka', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Esteka $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Esteka $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Esteka $model,
     ...
     * @param Klikasi\Model\Raw\Esteka|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Esteka The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Esteka();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Esteka) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Esteka');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Esteka');
        }
