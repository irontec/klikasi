...
/**
 * Data Mapper implementation for Klikasi\Model\KlearUsers
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
{
    protected $_modelName = 'Klikasi\\Model\\KlearUsers';
     ...
     *
     * @param Klikasi\Model\Raw\KlearUsers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\KlearUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\KlearUsers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\KlearUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\KlearUsers');
        }
     ...
     *
     * @param Klikasi\Model\Raw\KlearUsers $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\KlearUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\KlearUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\KlearUsers', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\KlearUsers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\KlearUsers $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\KlearUsers $model,
     ...
     * @param Klikasi\Model\Raw\KlearUsers|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\KlearUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\KlearUsers();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\KlearUsers) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KlearUsers');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('KlearUsers');
        }
