...
/**
 * Data Mapper implementation for Klikasi\Model\EdukiaRelKategoria
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class EdukiaRelKategoria extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\EdukiaRelKategoria';
     ...
     *
     * @param Klikasi\Model\Raw\EdukiaRelKategoria $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\EdukiaRelKategoria) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\EdukiaRelKategoria object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\EdukiaRelKategoria object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelKategoria
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelKategoria');
        }
     ...
     *
     * @param Klikasi\Model\Raw\EdukiaRelKategoria $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\EdukiaRelKategoria) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\EdukiaRelKategoria object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\EdukiaRelKategoria object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelKategoria', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\EdukiaRelKategoria $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\EdukiaRelKategoria $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\EdukiaRelKategoria $model,
     ...
     * @param Klikasi\Model\Raw\EdukiaRelKategoria|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\EdukiaRelKategoria The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\EdukiaRelKategoria();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\EdukiaRelKategoria) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'EdukiaRelKategoria');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('EdukiaRelKategoria');
        }
