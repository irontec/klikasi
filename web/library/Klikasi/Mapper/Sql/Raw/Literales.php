...
/**
 * Data Mapper implementation for Klikasi\Model\Literales
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Literales extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Literales';
     ...
     *
     * @param Klikasi\Model\Raw\Literales $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Literales) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Literales object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Literales object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                'id' => $model->getId(),
                'identificativo' => $model->getIdentificativo(),
                'literaleu' => $model->getLiteraleu(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Literales
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Literales');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Literales $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Literales) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Literales object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Literales object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Literales', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Literales $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Literales $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Literales $model,
     ...
     * @param Klikasi\Model\Raw\Literales|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Literales The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Literales();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setIdentificativo($data['identificativo'])
                  ->setLiteraleu($data['literaleu']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setIdentificativo($data->{'identificativo'})
                  ->setLiteraleu($data->{'literaleu'});

        } else if ($data instanceof \Klikasi\Model\Raw\Literales) {
            $entry->setId($data->getId())
                  ->setIdentificativo($data->getIdentificativo())
                  ->setLiteraleu($data->getLiteraleu());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Literales');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Literales');
        }
