...
/**
 * Data Mapper implementation for Klikasi\Model\Newsletter
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Newsletter extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Newsletter';
     ...
     *
     * @param Klikasi\Model\Raw\Newsletter $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Newsletter) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Newsletter object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Newsletter object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'send' => $model->getSend(),
                'sent' => $model->getSent(),
                'readBy' => $model->getReadBy(),
                'dateToSend' => $model->getDateToSend(),
                'shippingDate' => $model->getShippingDate(),
                'isEdukia' => $model->getIsEdukia(),
                'isIkastegia' => $model->getIsIkastegia(),
                'edukiakSent' => $model->getEdukiakSent(),
                'ikastegiakSent' => $model->getIkastegiakSent(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Newsletter
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Newsletter');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Newsletter $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Newsletter) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Newsletter object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Newsletter object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Newsletter', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Newsletter $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Newsletter $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Newsletter $model,
            ...
            if ($recursive) {
                if ($model->getNewsletterHash(null, null, true) !== null) {
                    ...
                    foreach ($newsletterHash as $value) {
                        $value->setNewsletterId($primaryKey)
     ...
     * @param Klikasi\Model\Raw\Newsletter|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Newsletter The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Newsletter();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setDateToSend($data['dateToSend'])
                  ->setShippingDate($data['shippingDate'])
                  ->setIsEdukia($data['isEdukia'])
                  ->setIsIkastegia($data['isIkastegia'])
                  ->setEdukiakSent($data['edukiakSent'])
                  ->setIkastegiakSent($data['ikastegiakSent']);
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Newsletter) {
            $entry->setId($data->getId())
                  ->setTitle($data->getTitle())
                  ->setContent($data->getContent())
                  ->setActive($data->getActive())
                  ->setSend($data->getSend())
                  ->setSent($data->getSent())
                  ->setReadBy($data->getReadBy())
                  ->setDateToSend($data->getDateToSend())
                  ->setShippingDate($data->getShippingDate())
                  ->setIsEdukia($data->getIsEdukia())
                  ->setIsIkastegia($data->getIsIkastegia())
                  ->setEdukiakSent($data->getEdukiakSent())
                  ->setIkastegiakSent($data->getIkastegiakSent());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Newsletter');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Newsletter');
        }
