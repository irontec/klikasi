...
/**
 * Data Mapper implementation for Klikasi\Model\Etiketa
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Etiketa extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Etiketa';
     ...
     *
     * @param Klikasi\Model\Raw\Etiketa $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Etiketa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Etiketa object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Etiketa object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Etiketa
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Etiketa');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Etiketa $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Etiketa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Etiketa object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Etiketa object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Etiketa', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Etiketa $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Etiketa $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Etiketa $model,
     ...
     * @param Klikasi\Model\Raw\Etiketa|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Etiketa The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Etiketa();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Etiketa) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Etiketa');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Etiketa');
        }
