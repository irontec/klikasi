...
class Jakinarazpenak extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Jakinarazpenak';
     ...
     *
     * @param Klikasi\Model\Raw\Jakinarazpenak $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Jakinarazpenak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Jakinarazpenak object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Jakinarazpenak object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'idKanpotarra' => $model->getIdKanpotarra(),
                'ikusita' => $model->getIkusita(),
                'message' => $model->getMessage(),
                'postazJakinarazi' => $model->getPostazJakinarazi(),
                'sortzeData' => $model->getSortzeData(),
                'thatUserId' => $model->getThatUserId(),
                'motaId' => $model->getMotaId(),
                'deletedBySender' => $model->getDeletedBySender(),
                'deletedByErabiltzailea' => $model->getDeletedByErabiltzailea(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Jakinarazpenak
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Jakinarazpenak');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Jakinarazpenak $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Jakinarazpenak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Jakinarazpenak object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Jakinarazpenak object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Jakinarazpenak', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Jakinarazpenak $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Jakinarazpenak $model, $useTransaction = true,
            ...
            if ($recursive) {
                if ($model->getJakinarazpenakGroup(null, null, true) !== null) {
                    ...
                    foreach ($jakinarazpenakGroup as $value) {
                        $value->setJakinarazpenakId($primaryKey)
     ...
     * @param Klikasi\Model\Raw\Jakinarazpenak|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Jakinarazpenak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Jakinarazpenak();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setIdKanpotarra($data['idKanpotarra'])
                  ->setIkusita($data['ikusita'])
                  ->setMessage($data['message'])
                  ->setPostazJakinarazi($data['postazJakinarazi'])
                  ->setSortzeData($data['sortzeData'])
                  ->setThatUserId($data['thatUserId'])
                  ->setMotaId($data['motaId'])
                  ->setDeletedBySender($data['deletedBySender'])
                  ->setDeletedByErabiltzailea($data['deletedByErabiltzailea'])
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Jakinarazpenak) {
            $entry->setId($data->getId())
                  ->setErabiltzaileaId($data->getErabiltzaileaId())
                  ->setIdKanpotarra($data->getIdKanpotarra())
                  ->setIkusita($data->getIkusita())
                  ->setMessage($data->getMessage())
                  ->setPostazJakinarazi($data->getPostazJakinarazi())
                  ->setSortzeData($data->getSortzeData())
                  ->setThatUserId($data->getThatUserId())
                  ->setMotaId($data->getMotaId())
                  ->setDeletedBySender($data->getDeletedBySender())
                  ->setDeletedByErabiltzailea($data->getDeletedByErabiltzailea())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Jakinarazpenak');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Jakinarazpenak');
        }
