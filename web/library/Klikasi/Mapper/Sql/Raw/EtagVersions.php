...
/**
 * Data Mapper implementation for Klikasi\Model\EtagVersions
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class EtagVersions extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\EtagVersions';
     ...
     *
     * @param Klikasi\Model\Raw\EtagVersions $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\EtagVersions) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\EtagVersions object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\EtagVersions object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\EtagVersions
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\EtagVersions');
        }
     ...
     *
     * @param Klikasi\Model\Raw\EtagVersions $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\EtagVersions) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\EtagVersions object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\EtagVersions object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\EtagVersions', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\EtagVersions $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\EtagVersions $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\EtagVersions $model,
     ...
     * @param Klikasi\Model\Raw\EtagVersions|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\EtagVersions The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\EtagVersions();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\EtagVersions) {
            $entry->setId($data->getId())
                  ->setTable($data->getTable())
                  ->setEtag($data->getEtag())
                  ->setLastChange($data->getLastChange());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'EtagVersions');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('EtagVersions');
        }
