...
/**
 * Data Mapper implementation for Klikasi\Model\KategoriakRelKategoriaGlobalak
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class KategoriakRelKategoriaGlobalak extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\KategoriakRelKategoriaGlobalak';
     ...
     *
     * @param Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\KategoriakRelKategoriaGlobalak object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                'id' => $model->getId(),
                'kategoriaId' => $model->getKategoriaId(),
                'kategoriaGlobalaId' => $model->getKategoriaGlobalaId(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\KategoriakRelKategoriaGlobalak
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\KategoriakRelKategoriaGlobalak');
        }
     ...
     *
     * @param Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\KategoriakRelKategoriaGlobalak object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\KategoriakRelKategoriaGlobalak object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\KategoriakRelKategoriaGlobalak', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak $model,
     ...
     * @param Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\KategoriakRelKategoriaGlobalak();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setKategoriaId($data['kategoriaId'])
                  ->setKategoriaGlobalaId($data['kategoriaGlobalaId']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setKategoriaId($data->{'kategoriaId'})
                  ->setKategoriaGlobalaId($data->{'kategoriaGlobalaId'});

        } else if ($data instanceof \Klikasi\Model\Raw\KategoriakRelKategoriaGlobalak) {
            $entry->setId($data->getId())
                  ->setKategoriaId($data->getKategoriaId())
                  ->setKategoriaGlobalaId($data->getKategoriaGlobalaId());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KategoriakRelKategoriaGlobalak');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('KategoriakRelKategoriaGlobalak');
        }
