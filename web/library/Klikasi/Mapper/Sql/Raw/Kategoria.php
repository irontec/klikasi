...
class Kategoria extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Kategoria';
     ...
     *
     * @param Klikasi\Model\Raw\Kategoria $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Kategoria) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Kategoria object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Kategoria object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Kategoria
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Kategoria');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Kategoria $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Kategoria) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Kategoria object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Kategoria object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Kategoria', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Kategoria $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Kategoria $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Kategoria $model,
            ...
            if ($recursive) {
                if ($model->getEdukiaRelKategoria(null, null, true) !== null) {
                    ...
                    foreach ($edukiaRelKategoria as $value) {
                        $value->setKategoriaId($primaryKey)
                ...
                if ($model->getKategoriakRelKategoriaGlobalak(null, null, true) !== null) {
                    $kategoriakRelKategoriaGlobalak = $model->getKategoriakRelKategoriaGlobalak();

                    if (!is_array($kategoriakRelKategoriaGlobalak)) {

                        $kategoriakRelKategoriaGlobalak = array($kategoriakRelKategoriaGlobalak);
                    }

                    foreach ($kategoriakRelKategoriaGlobalak as $value) {
                        $value->setKategoriaId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Klikasi\Model\Raw\Kategoria|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Kategoria The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Kategoria();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Kategoria) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Kategoria');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Kategoria');
        }
