...
class Iruzkina extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Iruzkina';
     ...
     *
     * @param Klikasi\Model\Raw\Iruzkina $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Iruzkina) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Iruzkina object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Iruzkina object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Iruzkina
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Iruzkina');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Iruzkina $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Iruzkina) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Iruzkina object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Iruzkina object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Iruzkina', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Iruzkina $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Iruzkina $model, $useTransaction = true,
            ...
            if ($recursive) {
                if ($model->getIruzkina(null, null, true) !== null) {
                    ...
                    foreach ($iruzkina as $value) {
                        $value->setIruzkinaId($primaryKey)
     ...
     * @param Klikasi\Model\Raw\Iruzkina|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Iruzkina The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Iruzkina();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Iruzkina) {
            $entry->setId($data->getId())
                  ->setIruzkinaId($data->getIruzkinaId())
                  ->setEdukiaId($data->getEdukiaId())
                  ->setErabiltzaileaId($data->getErabiltzaileaId())
                  ->setIruzkin($data->getIruzkin())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Iruzkina');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Iruzkina');
        }
