...
/**
 * Data Mapper implementation for Klikasi\Model\Fitxategia
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Fitxategia extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Fitxategia';
     ...
     *
     * @param Klikasi\Model\Raw\Fitxategia $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Fitxategia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Fitxategia object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Fitxategia object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                'id' => $model->getId(),
                'edukiaId' => $model->getEdukiaId(),
                'titulua' => $model->getTitulua(),
                'url' => $model->getUrl(),
                'fitxategiaFileSize' => $model->getFitxategiaFileSize(),
                'fitxategiaMimeType' => $model->getFitxategiaMimeType(),
                'fitxategiaBaseName' => $model->getFitxategiaBaseName(),
                'klikKopurua' => $model->getKlikKopurua(),
                'sortzeData' => $model->getSortzeData(),
                'mota' => $model->getMota(),
                'downloadMota' => $model->getDownloadMota(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Fitxategia
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Fitxategia');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Fitxategia $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Fitxategia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Fitxategia object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Fitxategia object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Fitxategia', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Fitxategia $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Fitxategia $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Fitxategia $model,
     ...
     * @param Klikasi\Model\Raw\Fitxategia|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Fitxategia The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Fitxategia();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setEdukiaId($data['edukiaId'])
                  ->setTitulua($data['titulua'])
                  ->setUrl($data['url'])
                  ->setFitxategiaFileSize($data['fitxategiaFileSize'])
                  ->setFitxategiaMimeType($data['fitxategiaMimeType'])
                  ->setFitxategiaBaseName($data['fitxategiaBaseName'])
                  ->setKlikKopurua($data['klikKopurua'])
                  ->setSortzeData($data['sortzeData'])
                  ->setMota($data['mota'])
                  ->setDownloadMota($data['downloadMota']);
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Fitxategia) {
            $entry->setId($data->getId())
                  ->setEdukiaId($data->getEdukiaId())
                  ->setTitulua($data->getTitulua())
                  ->setUrl($data->getUrl())
                  ->setFitxategiaFileSize($data->getFitxategiaFileSize())
                  ->setFitxategiaMimeType($data->getFitxategiaMimeType())
                  ->setFitxategiaBaseName($data->getFitxategiaBaseName())
                  ->setKlikKopurua($data->getKlikKopurua())
                  ->setSortzeData($data->getSortzeData())
                  ->setMota($data->getMota())
                  ->setDownloadMota($data->getDownloadMota());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Fitxategia');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Fitxategia');
        }
