...
/**
 * Data Mapper implementation for Klikasi\Model\Aurkezpena
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Aurkezpena extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Aurkezpena';
     ...
     *
     * @param Klikasi\Model\Raw\Aurkezpena $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Aurkezpena) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Aurkezpena object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Aurkezpena object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Aurkezpena
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Aurkezpena');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Aurkezpena $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Aurkezpena) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Aurkezpena object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Aurkezpena object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Aurkezpena', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Aurkezpena $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Aurkezpena $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Aurkezpena $model,
     ...
     * @param Klikasi\Model\Raw\Aurkezpena|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Aurkezpena The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Aurkezpena();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Aurkezpena) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Aurkezpena');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Aurkezpena');
        }
