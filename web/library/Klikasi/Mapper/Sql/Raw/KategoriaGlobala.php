...
/**
 * Data Mapper implementation for Klikasi\Model\KategoriaGlobala
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class KategoriaGlobala extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\KategoriaGlobala';
     ...
     *
     * @param Klikasi\Model\Raw\KategoriaGlobala $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\KategoriaGlobala) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\KategoriaGlobala object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\KategoriaGlobala object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\KategoriaGlobala
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\KategoriaGlobala');
        }
     ...
     *
     * @param Klikasi\Model\Raw\KategoriaGlobala $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\KategoriaGlobala) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\KategoriaGlobala object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\KategoriaGlobala object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\KategoriaGlobala', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\KategoriaGlobala $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\KategoriaGlobala $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\KategoriaGlobala $model,
            ...
            if ($recursive) {
                if ($model->getKategoriakRelKategoriaGlobalak(null, null, true) !== null) {
                    ...
                    foreach ($kategoriakRelKategoriaGlobalak as $value) {
                        $value->setKategoriaGlobalaId($primaryKey)
     ...
     * @param Klikasi\Model\Raw\KategoriaGlobala|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\KategoriaGlobala The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\KategoriaGlobala();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\KategoriaGlobala) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KategoriaGlobala');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('KategoriaGlobala');
        }
