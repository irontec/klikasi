...
/**
 * Data Mapper implementation for Klikasi\Model\Notifikazioa
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Notifikazioa extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Notifikazioa';
     ...
     *
     * @param Klikasi\Model\Raw\Notifikazioa $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Notifikazioa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Notifikazioa object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Notifikazioa object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'edukiaHtml' => $model->getEdukiaHtml(),
                'edukiaText' => $model->getEdukiaText(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Notifikazioa
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Notifikazioa');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Notifikazioa $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Notifikazioa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Notifikazioa object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Notifikazioa object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Notifikazioa', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Notifikazioa $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Notifikazioa $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Notifikazioa $model,
     ...
     * @param Klikasi\Model\Raw\Notifikazioa|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Notifikazioa The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Notifikazioa();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setEdukiaHtml($data['edukiaHtml'])
                  ->setEdukiaText($data['edukiaText'])
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Notifikazioa) {
            $entry->setId($data->getId())
                  ->setTitulua($data->getTitulua())
                  ->setEdukiaHtml($data->getEdukiaHtml())
                  ->setEdukiaText($data->getEdukiaText())
                  ->setIdentifikatzailea($data->getIdentifikatzailea());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Notifikazioa');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Notifikazioa');
        }
