...
/**
 * Data Mapper implementation for Klikasi\Model\Orrialdea
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class Orrialdea extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Orrialdea';
     ...
     *
     * @param Klikasi\Model\Raw\Orrialdea $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Orrialdea) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Orrialdea object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Orrialdea object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'publikatua' => $model->getPublikatua(),
                'url' => $model->getUrl(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Orrialdea
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Orrialdea');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Orrialdea $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Orrialdea) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Orrialdea object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Orrialdea object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Orrialdea', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Orrialdea $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Orrialdea $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\Orrialdea $model,
     ...
     * @param Klikasi\Model\Raw\Orrialdea|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Orrialdea The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Orrialdea();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Orrialdea) {
            $entry->setId($data->getId())
                  ->setTitulua($data->getTitulua())
                  ->setEdukia($data->getEdukia())
                  ->setPublikatua($data->getPublikatua())
                  ->setUrl($data->getUrl())
                  ->setSortzeData($data->getSortzeData())
                  ->setMetaTitle($data->getMetaTitle())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Orrialdea');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Orrialdea');
        }
