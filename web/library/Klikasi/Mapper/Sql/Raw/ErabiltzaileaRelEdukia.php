...
class ErabiltzaileaRelEdukia extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\ErabiltzaileaRelEdukia';
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileaRelEdukia $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileaRelEdukia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\ErabiltzaileaRelEdukia object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\ErabiltzaileaRelEdukia object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelEdukia
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelEdukia');
        }
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileaRelEdukia $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileaRelEdukia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\ErabiltzaileaRelEdukia object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\ErabiltzaileaRelEdukia object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelEdukia', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\ErabiltzaileaRelEdukia $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\ErabiltzaileaRelEdukia $model, $useTransaction = true,
     ...
     * @param Klikasi\Model\Raw\ErabiltzaileaRelEdukia|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\ErabiltzaileaRelEdukia The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\ErabiltzaileaRelEdukia();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\ErabiltzaileaRelEdukia) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ErabiltzaileaRelEdukia');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('ErabiltzaileaRelEdukia');
        }
