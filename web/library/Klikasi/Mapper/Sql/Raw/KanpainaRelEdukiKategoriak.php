...
/**
 * Data Mapper implementation for Klikasi\Model\KanpainaRelEdukiKategoriak
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class KanpainaRelEdukiKategoriak extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\KanpainaRelEdukiKategoriak';
     ...
     *
     * @param Klikasi\Model\Raw\KanpainaRelEdukiKategoriak $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\KanpainaRelEdukiKategoriak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\KanpainaRelEdukiKategoriak object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\KanpainaRelEdukiKategoriak object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                'id' => $model->getId(),
                'kanpainaId' => $model->getKanpainaId(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelEdukiKategoriak
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelEdukiKategoriak');
        }
     ...
     *
     * @param Klikasi\Model\Raw\KanpainaRelEdukiKategoriak $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\KanpainaRelEdukiKategoriak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\KanpainaRelEdukiKategoriak object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\KanpainaRelEdukiKategoriak object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\KanpainaRelEdukiKategoriak', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\KanpainaRelEdukiKategoriak $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\KanpainaRelEdukiKategoriak $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\KanpainaRelEdukiKategoriak $model,
     ...
     * @param Klikasi\Model\Raw\KanpainaRelEdukiKategoriak|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\KanpainaRelEdukiKategoriak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\KanpainaRelEdukiKategoriak();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\KanpainaRelEdukiKategoriak) {
            $entry->setId($data->getId())
                  ->setKanpainaId($data->getKanpainaId())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KanpainaRelEdukiKategoriak');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('KanpainaRelEdukiKategoriak');
        }
