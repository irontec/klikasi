...
/**
 * Data Mapper implementation for Klikasi\Model\EmailHistory
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class EmailHistory extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\EmailHistory';
     ...
     *
     * @param Klikasi\Model\Raw\EmailHistory $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\EmailHistory) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\EmailHistory object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\EmailHistory object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\EmailHistory
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\EmailHistory');
        }
     ...
     *
     * @param Klikasi\Model\Raw\EmailHistory $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\EmailHistory) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\EmailHistory object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\EmailHistory object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\EmailHistory', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\EmailHistory $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\EmailHistory $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\EmailHistory $model,
     ...
     * @param Klikasi\Model\Raw\EmailHistory|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\EmailHistory The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\EmailHistory();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\EmailHistory) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'EmailHistory');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('EmailHistory');
        }
