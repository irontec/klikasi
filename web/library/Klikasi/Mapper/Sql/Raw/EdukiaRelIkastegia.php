...
class EdukiaRelIkastegia extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\EdukiaRelIkastegia';
     ...
     *
     * @param Klikasi\Model\Raw\EdukiaRelIkastegia $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\EdukiaRelIkastegia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\EdukiaRelIkastegia object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\EdukiaRelIkastegia object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelIkastegia
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelIkastegia');
        }
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\EdukiaRelIkastegia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\EdukiaRelIkastegia object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\EdukiaRelIkastegia object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelIkastegia', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\EdukiaRelIkastegia $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\EdukiaRelIkastegia $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\EdukiaRelIkastegia $model,
     ...
     * @param Klikasi\Model\Raw\EdukiaRelIkastegia|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\EdukiaRelIkastegia The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\EdukiaRelIkastegia();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\EdukiaRelIkastegia) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'EdukiaRelIkastegia');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('EdukiaRelIkastegia');
        }
