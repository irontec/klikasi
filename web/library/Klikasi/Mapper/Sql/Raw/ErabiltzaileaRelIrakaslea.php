...
class ErabiltzaileaRelIrakaslea extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\ErabiltzaileaRelIrakaslea';
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\ErabiltzaileaRelIrakaslea object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'irakasleaId' => $model->getIrakasleaId(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelIrakaslea
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelIrakaslea');
        }
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\ErabiltzaileaRelIrakaslea object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\ErabiltzaileaRelIrakaslea object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelIrakaslea', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea $model, $useTransaction = true,
     ...
     * @param Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\ErabiltzaileaRelIrakaslea();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setIrakasleaId($data['irakasleaId'])
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\ErabiltzaileaRelIrakaslea) {
            $entry->setId($data->getId())
                  ->setErabiltzaileaId($data->getErabiltzaileaId())
                  ->setIrakasleaId($data->getIrakasleaId())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ErabiltzaileaRelIrakaslea');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('ErabiltzaileaRelIrakaslea');
        }
