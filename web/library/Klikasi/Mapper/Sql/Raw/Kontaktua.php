...
{
    protected $_modelName = 'Klikasi\\Model\\Kontaktua';
     ...
     *
     * @param Klikasi\Model\Raw\Kontaktua $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Kontaktua) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Kontaktua object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Kontaktua object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Kontaktua
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Kontaktua');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Kontaktua $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Kontaktua) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Kontaktua object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Kontaktua object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Kontaktua', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Kontaktua $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Kontaktua $model, $useTransaction = true,
     ...
     * @param Klikasi\Model\Raw\Kontaktua|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Kontaktua The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Kontaktua();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Kontaktua) {
            $entry->setId($data->getId())
                  ->setIzena($data->getIzena())
                  ->setPosta($data->getPosta())
                  ->setGaia($data->getGaia())
                  ->setNondik($data->getNondik())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Kontaktua');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Kontaktua');
        }
