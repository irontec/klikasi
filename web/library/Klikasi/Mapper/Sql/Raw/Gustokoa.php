...
class Gustokoa extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\Gustokoa';
     ...
     *
     * @param Klikasi\Model\Raw\Gustokoa $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\Gustokoa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\Gustokoa object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\Gustokoa object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\Gustokoa
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\Gustokoa');
        }
     ...
     *
     * @param Klikasi\Model\Raw\Gustokoa $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\Gustokoa) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\Gustokoa object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\Gustokoa object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\Gustokoa', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\Gustokoa $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\Gustokoa $model, $useTransaction = true,
     ...
     * @param Klikasi\Model\Raw\Gustokoa|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\Gustokoa The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\Gustokoa();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\Gustokoa) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Gustokoa');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('Gustokoa');
        }
