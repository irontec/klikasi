...
/**
 * Data Mapper implementation for Klikasi\Model\IkastegiMota
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class IkastegiMota extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\IkastegiMota';
     ...
     *
     * @param Klikasi\Model\Raw\IkastegiMota $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\IkastegiMota) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\IkastegiMota object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\IkastegiMota object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\IkastegiMota
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\IkastegiMota');
        }
     ...
     *
     * @param Klikasi\Model\Raw\IkastegiMota $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\IkastegiMota) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\IkastegiMota object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\IkastegiMota object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\IkastegiMota', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\IkastegiMota $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\IkastegiMota $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\IkastegiMota $model,
            ...
            if ($recursive) {
                if ($model->getIkastegiaRelMota(null, null, true) !== null) {
                    ...
                    foreach ($ikastegiaRelMota as $value) {
                        $value->setIkastegiMotaId($primaryKey)
                ...
                if ($model->getKanpainaRelIkastegiMota(null, null, true) !== null) {
                    ...
                    foreach ($kanpainaRelIkastegiMota as $value) {
                        $value->setIkastetxeMotaId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Klikasi\Model\Raw\IkastegiMota|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\IkastegiMota The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\IkastegiMota();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\IkastegiMota) {
            $entry->setId($data->getId())
                  ->setIzena($data->getIzena())
                  ->setUrl($data->getUrl())
                  ->setDefault($data->getDefault());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'IkastegiMota');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('IkastegiMota');
        }
