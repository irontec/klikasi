...
class ErabiltzaileaRelIkastegia extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\ErabiltzaileaRelIkastegia';
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileaRelIkastegia $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileaRelIkastegia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\ErabiltzaileaRelIkastegia object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\ErabiltzaileaRelIkastegia object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'administratzailea' => $model->getAdministratzailea(),
                'jabea' => $model->getJabea(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelIkastegia
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelIkastegia');
        }
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileaRelIkastegia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\ErabiltzaileaRelIkastegia object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\ErabiltzaileaRelIkastegia object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileaRelIkastegia', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\ErabiltzaileaRelIkastegia $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\ErabiltzaileaRelIkastegia $model, $useTransaction = true,
     ...
     * @param Klikasi\Model\Raw\ErabiltzaileaRelIkastegia|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\ErabiltzaileaRelIkastegia The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\ErabiltzaileaRelIkastegia();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setAdministratzailea($data['administratzailea'])
                  ->setJabea($data['jabea'])
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\ErabiltzaileaRelIkastegia) {
            $entry->setId($data->getId())
                  ->setErabiltzaileaId($data->getErabiltzaileaId())
                  ->setIkastegiaId($data->getIkastegiaId())
                  ->setAdministratzailea($data->getAdministratzailea())
                  ->setJabea($data->getJabea())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ErabiltzaileaRelIkastegia');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('ErabiltzaileaRelIkastegia');
        }
