...
/**
 * Data Mapper implementation for Klikasi\Model\ErabiltzaileenIrudiak
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class ErabiltzaileenIrudiak extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\ErabiltzaileenIrudiak';
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileenIrudiak $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileenIrudiak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\ErabiltzaileenIrudiak object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\ErabiltzaileenIrudiak object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'partekatua' => $model->getPartekatua(),
                'iden' => $model->getIden(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileenIrudiak
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileenIrudiak');
        }
     ...
     *
     * @param Klikasi\Model\Raw\ErabiltzaileenIrudiak $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\ErabiltzaileenIrudiak) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\ErabiltzaileenIrudiak object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\ErabiltzaileenIrudiak object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\ErabiltzaileenIrudiak', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\ErabiltzaileenIrudiak $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\ErabiltzaileenIrudiak $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\ErabiltzaileenIrudiak $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getErabiltzailea(null, null, true) !== null) {
                    $erabiltzailea = $model->getErabiltzailea();

                    if (!is_array($erabiltzailea)) {

                        $erabiltzailea = array($erabiltzailea);
                    }

                    foreach ($erabiltzailea as $value) {
                        $value->setIrudiaId($primaryKey)
     ...
     * @param Klikasi\Model\Raw\ErabiltzaileenIrudiak|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\ErabiltzaileenIrudiak The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\ErabiltzaileenIrudiak();
        }
        ...
        if (is_array($data)) {
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\ErabiltzaileenIrudiak) {
            $entry->setId($data->getId())
                  ...
                  ->setPartekatua($data->getPartekatua())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ErabiltzaileenIrudiak');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('ErabiltzaileenIrudiak');
        }
