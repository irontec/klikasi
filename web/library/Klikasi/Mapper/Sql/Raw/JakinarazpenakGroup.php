...
/**
 * Data Mapper implementation for Klikasi\Model\JakinarazpenakGroup
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class JakinarazpenakGroup extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\JakinarazpenakGroup';
     ...
     *
     * @param Klikasi\Model\Raw\JakinarazpenakGroup $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\JakinarazpenakGroup) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\JakinarazpenakGroup object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\JakinarazpenakGroup object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                'id' => $model->getId(),
                'jakinarazpenakId' => $model->getJakinarazpenakId(),
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\JakinarazpenakGroup
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\JakinarazpenakGroup');
        }
     ...
     *
     * @param Klikasi\Model\Raw\JakinarazpenakGroup $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\JakinarazpenakGroup) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\JakinarazpenakGroup object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\JakinarazpenakGroup object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\JakinarazpenakGroup', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\JakinarazpenakGroup $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\JakinarazpenakGroup $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\JakinarazpenakGroup $model,
     ...
     * @param Klikasi\Model\Raw\JakinarazpenakGroup|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\JakinarazpenakGroup The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\JakinarazpenakGroup();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setJakinarazpenakId($data['jakinarazpenakId'])
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\JakinarazpenakGroup) {
            $entry->setId($data->getId())
                  ->setJakinarazpenakId($data->getJakinarazpenakId())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'JakinarazpenakGroup');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('JakinarazpenakGroup');
        }
