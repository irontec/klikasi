...
class IkastegiGaiak extends MapperAbstract
{
     ...
     */
    public function save(\Klikasi\Model\Raw\IkastegiGaiak $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\IkastegiGaiak $model, $useTransaction = true,
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'IkastegiGaiak');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('IkastegiGaiak');
        }
