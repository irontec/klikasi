...
/**
 * Data Mapper implementation for Klikasi\Model\EdukiaRelKanpaina
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class EdukiaRelKanpaina extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\EdukiaRelKanpaina';
     ...
     *
     * @param Klikasi\Model\Raw\EdukiaRelKanpaina $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\EdukiaRelKanpaina) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\EdukiaRelKanpaina object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\EdukiaRelKanpaina object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelKanpaina
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelKanpaina');
        }
     ...
     *
     * @param Klikasi\Model\Raw\EdukiaRelKanpaina $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\EdukiaRelKanpaina) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\EdukiaRelKanpaina object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\EdukiaRelKanpaina object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\EdukiaRelKanpaina', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\EdukiaRelKanpaina $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\EdukiaRelKanpaina $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\EdukiaRelKanpaina $model,
     ...
     * @param Klikasi\Model\Raw\EdukiaRelKanpaina|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\EdukiaRelKanpaina The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\EdukiaRelKanpaina();
        }
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\EdukiaRelKanpaina) {
            $entry->setId($data->getId())
                  ->setKanpainaId($data->getKanpainaId())
                  ->setEdukiaId($data->getEdukiaId())
                  ->setIrabazlea($data->getIrabazlea());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'EdukiaRelKanpaina');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('EdukiaRelKanpaina');
        }
