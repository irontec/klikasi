...
/**
 * Data Mapper implementation for Klikasi\Model\JakinarazpenakMota
 *
 * @package Klikasi\Mapper\Sql
 * @subpackage Raw
 * @author Irontec
...
class JakinarazpenakMota extends MapperAbstract
{
    protected $_modelName = 'Klikasi\\Model\\JakinarazpenakMota';
     ...
     *
     * @param Klikasi\Model\Raw\JakinarazpenakMota $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Klikasi\Model\Raw\JakinarazpenakMota) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Klikasi\Model\Raw\JakinarazpenakMota object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\Model\\JakinarazpenakMota object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'url' => $model->getUrl(),
                'deskripzioa' => $model->getDeskripzioa(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Klikasi\\Mapper\\Sql\\DbTable\\JakinarazpenakMota
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Klikasi\\Mapper\\Sql\\DbTable\\JakinarazpenakMota');
        }
     ...
     *
     * @param Klikasi\Model\Raw\JakinarazpenakMota $model The model to delete
    ...
    public function delete(\Klikasi\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Klikasi\Model\Raw\JakinarazpenakMota) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Klikasi\\Model\\JakinarazpenakMota object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Klikasi\\Model\\JakinarazpenakMota object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Klikasi\\Mapper\\Sql\\DbTable\\JakinarazpenakMota', $capitalizedFk);
     ...
     */
    public function save(\Klikasi\Model\Raw\JakinarazpenakMota $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Klikasi\Model\Raw\JakinarazpenakMota $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Klikasi\Model\Raw\JakinarazpenakMota $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getJakinarazpenak(null, null, true) !== null) {
                    $jakinarazpenak = $model->getJakinarazpenak();
     ...
     * @param Klikasi\Model\Raw\JakinarazpenakMota|null $entry The object to load the data into, or null to have one created
     * @return Klikasi\Model\Raw\JakinarazpenakMota The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Klikasi\Model\JakinarazpenakMota();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setIzena($data['izena'])
                  ->setUrl($data['url'])
                  ->setDeskripzioa($data['deskripzioa']);
        ...
        } else if ($data instanceof \Klikasi\Model\Raw\JakinarazpenakMota) {
            $entry->setId($data->getId())
                  ->setIzena($data->getIzena())
                  ->setUrl($data->getUrl())
                  ->setDeskripzioa($data->getDeskripzioa());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'JakinarazpenakMota');

        if (empty($etag)) {
            $etag = new \Klikasi\Model\EtagVersions();
            $etag->setTable('JakinarazpenakMota');
        }
